...
| `visual` | `visual` | 実装。scene.json → visual-synth → **visual-loop.mp4**（ループ素材。§4.3） |
| `video` | `video` | 実装。`-stream_loop` で尺まで伸ばして ffmpeg合成＋ラウドネス正規化 → final.mp4 |
| `metadata` | `metadata` | 実装。LLM → metadata.json |
| — | `checks` | **新規**。効能リンター / uniqueness重複 / ラウドネス検査 |
| `humanReview` | `review` | 置換。チェック結果＋動画リンクのサマリ出力 |
| `publish` | `upload` | 実装。限定公開アップロード。**Content IDクレーム確認は含まない**（§5.8。API不可のため `review` の人間ゲートに落ちる） |
...
| `visual-loop.mp4` | visual | **完全ループ素材・映像のみ**（尺全体ではない。§4.3） |
| `final.mp4` | video | 映像×音声・ラウドネス正規化済み。ループを `-stream_loop` で尺まで伸ばす（§4.3） |
| `metadata.json` | metadata | タイトル・説明・タグ・サムネ案 |
| `checks.json` | checks | 自動チェック結果 |
| `upload.json` | upload | videoId・URL・APIレスポンス。**Content IDクレーム情報は書けない**（§5.8。API不可） |
...
| P0-4 | `packages/visual-synth` MVP（three.jsヘッドレス1シーン・完全ループ → visual-loop.mp4） | P0-1, §7確定 |
...
| P0-8 | `upload` ステージ（OAuth2・限定公開・AI開示ON・`--dry-run`） | P0-7 |
